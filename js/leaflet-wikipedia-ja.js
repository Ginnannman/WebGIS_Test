L.Wikipedia = L.FeatureGroup.extend({
	options: { // AND {language}
		urlTemplate: 'http://ja.dbpedia.org/sparql/?default-graph-uri={defaultGraphUri}&query={query}&format={format}',
		...
		params: {
			format: 'json',
			defaultGraphUri: 'http://ja.dbpedia.org', // ???			
		},
		fields: {
			abstract: [
				"?abstract",
				"?res <http://ja.dbpedia.org/ontology/abstract> ?abstract.",
			...
			thumbnail: [
				"?thumbnail",
				"?res <http://ja.dbpedia.org/ontology/thumbnail> ?thumbnail."
			],
			type: [
